...
glob("./src/documents/json-files/track3/*.json", {}, async(err, files)=>{
  ...
  files.forEach(file=>{
    fs.readFile(file, async(err,data)=>{
      ...
      Object.keys(slides).forEach((slideNum)=>{
        let slideMeta = {}
        if(slides[slideNum].actions)
          slideMeta.actions = slides[slideNum].actions
        if(slides[slideNum].tips)
          slideMeta.tips = slides[slideNum].tips
        
        let error = false

        const slideMetadata = JSON.stringify(slideMeta)
        const slideUpsert = authedApollo.mutate({
          mutation: gql`
          ...
          `, variables: {
            input: {
              name: slides[slideNum].name,
              key: `${lessonKey}:slide${slideNum}`,
              description: slides[slideNum].description || "",
              priority: 0,
        ...
        }).catch((err)=>console.log("Slide Error: ", err.networkError) )
      })
    })
  })
})
